								...
								<Link to={`/listen/heds/hedstape/${index + 1}`}>
									<div className="flex-1 flex flex-col items-start p-2">
										...
										<div className="flex flex-col items-start justify-evenly px-2 pb-2">
											<h3 className="mt-4 text-neutral-975 dark:text-neutral-300 text-sm font-medium">{sample.artist}</h3>
											<dl className="mt-1 flex-grow flex flex-col justify-between">
												<dd className="text-neutral-700 dark:text-gray-400 text-sm">{sample.wallet.slice(0, 5) + "..."}</dd>
											</dl>
											<span className="text-gray-700 dark:text-gray-400 text-xs mt-1 font-semibold">{`hedsTAPE 0${index + 1}`}</span>
										</div>
									</div>
								</Link>
							</li>

...
const Spotlight = ({ exploreData }: { exploreData: ExploreState }) => {
    const { space, tape, id } = exploreData.spotlight;
    const tapeData = useSelector((state: RootState) => state.tapeModel);
    const currentTape = tapeData?.tapes?.[tape]?.[id];
    return (
        <Fragment>
            {tapeData && currentTape && (
                                ...
                                <div className="lg:col-span-1 h-full flex items-center justify-center lg:justify-start">
                                    <div className="flex flex-col justify-center items-center lg:items-start px-20 mt-0 lg:mt-5">
                                        <div className="flex -space-x-2 overflow-hidden mb-4">
                                            <img
                                                src={
                                                    currentTape?.sample?.image ||
                                                    currentTape?.collab?.curator
                                                }
                                                className="h-16 lg:h-20 w-16 lg:w-20 inline-block rounded-full"
                                            />
                                            <img
                                                src={HedDot}
                                                className="h-16 lg:h-20 w-16 lg:w-20 inline-block rounded-full bg-neutral-900 p-2 xl:p-3"
                                            />
                                            <img
                                                src={currentTape?.tape?.image}
                                                className="h-16 lg:h-20 w-16 lg:w-20 inline-block rounded-full lg:hidden lg:opacity-0"
                                            />
                                        </div>
                                        ...
                                        <LinkButton
                                            bg="bg-neutral-200 dark:bg-neutral-800 bg-opacity-100 dark:hover:bg-neutral-900 rounded-md transition-all"
                                            link={`/listen/${space}/${tape}/${currentTape?.tape?.no}`}>

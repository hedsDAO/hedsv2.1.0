...
const TrackDetails = () => {
    const { currentTape, currentTrack, currentTapeId, isSample } = useSelector(
    ...
    return (
        <div className="flex flex-col items-start justify-center animate__animated animate__fadeIn animate__fast">
            {trackData?.tracks && spaceData && audioData && (
                <>
                    <span className="text-neutral-800 dark:text-neutral-300 text-sm lg:text-xs font-base whitespace-nowrap">
                        {trackData?.tapes?.[currentTape]?.[currentTapeId] && (
                            <Link
                                to={trackData?.tapes?.[currentTape]?.[currentTapeId]?.links?.route}>
                                {isSample
                    ...
                    <span className="text-neutral-800 dark:text-neutral-400 text-sm lg:text-sm font-thin whitespace-nowrap">
                        {audioData?.isSample
                            ? `#0`
                            : trackData?.tracks?.[currentTape]?.[currentTapeId]?.[currentTrack]
                    ...
                    <span className="text-neutral-700 dark:text-neutral-500 text-xs lg:text-sm font-extralight whitespace-nowrap">
                        {isSample
                            ? trackData?.tapes?.[currentTape]?.[currentTapeId]?.sample?.artist
                            : trackData?.tracks?.[currentTape]?.[currentTapeId]?.[currentTrack]
                                  ?.artist}
                    </span>
                </>

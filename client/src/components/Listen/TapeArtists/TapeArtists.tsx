...
const TapeArtists = (trackData: [TrackMetadata]) => {
    ...
    const { currentTrack, currentTape, currentTapeId, isLoading } = useSelector(
                    ...
                    <div className="w-full bg-gray-300 dark:bg-neutral-975 rounded-lg xl:rounded-lg mx-auto shadow-sm">
                        <div className="grid grid-cols-12 place-items-center rounded-md gap-y-1 py-2 mx-2">
                            ...
                            {Object.values(trackData)?.length
                                ? Object.values(trackData).map(
                                      (track: TrackMetadata, i: number) => {
                                          return (
                                              <div
                                                  onClick={() => playTrack(i)}
                                                  key={track?.profilePicture}
                                                  className={
                                                      tapeData?.tracks?.[currentTape]?.[
                                                          currentTapeId
                                                  ...
                                                  <Fragment>
                                                      {track.title ? (
                                                          <>
                                                              ...
                                                              <div className="text-neutral-600 dark:text-neutral-500 col-span-2 lg:col-span-4 px-1 my-auto text-xs uppercase truncate overflow-hidden text-ellipsis">
                                                                  {track.title}
                                                              </div>
                                                          </>
                                                      ) : (
                                                          <>
                                                              <div className="text-neutral-800 dark:text-neutral-500 col-span-1 font-thin px-1">
                                                                  {i + 1}
                                                              </div>
                                                      ...
                                                      <div className="text-neutral-800 dark:text-neutral-500 font-thin uppercase tracking-widest col-span-3 lg:col-span-2 ml-auto text-xs lg:text-sm inline-flex items-center justify-end px-1 min-w-[4.5ch] max-w-[4.5ch]">
                                ...
                                : Array.apply(null, Array(tapeLength)).map(
                                      // @ts-ignore
                                      (el: any, index: number) => {
                                          return (
                                              <div
                                                  ...
                                                  <div className="col-span-1 font-thin text-neutral-600 text-xs px-1">
                                                      {index + 1}
                                                  </div>

...
const WarningModal = ({ title, message }: { title: string; message: string }) => {
	const { open } = useSelector((state: RootState) => state.globalModel.modal);
	const dispatch = useDispatch<Dispatch>();

	return (
		<Transition appear show={open} as={Fragment}>
			<Dialog as="div" className="relative z-[60]" onClose={() => dispatch.globalModel.setModalVisibility(false)}>
								...
								<div className="relative z-50 inline-block align-bottom bg-neutral-950 border-[0.25px] border-neutral-700 rounded-lg py-4 px-5 text-left overflow-hidden shadow-xl transform transition-all sm:align-middle max-w-full sm:max-w-md sm:w-full">
									<main className="max-w-lg mx-auto">
										<div className="flex flex-col h-full items-center justify-center py-5 mt-1">
											<p className="text-sm font-semibold text-red-500 mb-4 uppercase tracking-widest">
												<i className="fa-solid fa-circle-exclamation text-red-500 mr-2"></i>
												{title}
											</p>
											<div className="text-neutral-400 uppercase text-xs">{message}</div>
										</div>
									</main>
								</div>
							</Dialog.Panel>

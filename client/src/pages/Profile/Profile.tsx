...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        if (!userData?.isTapeArtist)
            dispatch.userModel.getTapeArtistsWalletIds(user?.attributes?.ethAddress.toLowerCase());
        if (!userData?.isVinylAddress)
            dispatch.userModel.getVinylAddress(user?.attributes?.ethAddress.toLowerCase());
    }, [userData]);
    useEffect(() => {

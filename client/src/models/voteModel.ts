...
import { UserFormattedOwnership } from "../models/common";
...
export const voteModel = createModel<RootModel>()({
    ...
    setVotingPower: (state, userData: SetVotingPower) => {
      ...
      const { userCollection, walletId, powerMapping } = userData;
      if (!userCollection) return newState;
      if (whitelist.includes(walletId)) newState.votingPower += 10;
      userCollection.map((tape, idx) => {
